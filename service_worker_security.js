...
global.caches = {
  ...
  async keys() { return ['old-cache', 'phactoryfit-v1.12.0']; },
...
(async () => {
  ...
  assert(addAll[1].includes('./zxing-browser.min.js?v=1.12.0'), 'scanner must be in the required shell');
  assert(addAll[1].includes('./app.js?v=1.12.0'), 'app code must be in the required shell');
  assert(addAll[1].includes('./restaurant-foods-expanded.js?v=1.12.0'), 'expanded restaurant catalog must be in the required shell');
  ...
  assert.equal(codePut[1], 'https://example.test/phactoryfit/app.js?v=1.12.0');
  ...
  console.log('PASSED 6/6 service-worker security checks');
})().catch(error => {
  console.error(error);
  process.exit(1);
});
