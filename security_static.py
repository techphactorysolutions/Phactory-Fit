...
check("scanner is in required offline shell", "'./zxing-browser.min.js?v=1.8.0'" in worker.split("const OPTIONAL_SHELL", 1)[0])
check("restaurant catalog is in required offline shell", "'./restaurant-foods.js?v=1.8.0'" in worker.split("const OPTIONAL_SHELL", 1)[0])
check("restaurant catalog is same-origin script", 'src="restaurant-foods.js?v=1.8.0"' in index)
